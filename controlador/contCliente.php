...
function controlador($proceso){
    ...
    switch ($proceso){
        case "NUEVO":
            $retorno = array();
            try{
                $duplicado = $objClie->verificarDuplicado(trim($_POST['nrodocumento']));
                if($duplicado->rowCount()==0){
                    $objClie->insertar(trim($_POST['nombre']), $_POST['idtipodocumento'], 
                                    $_POST['nrodocumento'], $_POST['direccion'],$_POST['estado']);                 
        ...
        case "ACTUALIZAR":
            $retorno = array();
            try{
                $duplicado = $objClie->verificarDuplicado(trim($_POST['nrodocumento']),$_POST['idcliente']);
                if($duplicado->rowCount()==0){
                    $objClie->actualizar($_POST['idcliente'],trim($_POST['nombre']),$_POST['idtipodocumento'], $_POST['nrodocumento'], $_POST['direccion'], $_POST['estado']);                 
                    $retorno['correcto']=true;
            ...
            case "CAMBIAR_ESTADO":
                $retorno = array();
                try{
                    $objClie->cambiarEstado($_POST['idcliente'],$_POST['estado']);                 
                    $retorno['correcto']=true;
        ...
        case 'CONSULTAR':
            $retorno = array();
            try{
                $registro = $objClie->consultar($_POST['idcliente']);
