...
class Venta{
    ...
    function listar($desde, $hasta, $cliente){
        $sql = "SELECT v.idventa, DATE_FORMAT(v.fecha,'%d/%m/%Y') fecha, v.fecha fechasort, v.serie, v.correlativo, 
                    v.total, tc.nombre comprobante,
    ...
    function insertarDetalle($detalle){
        $sql = "INSERT INTO detalle(iddetalle, idventa, idproducto, cantidad, unidad,
                            ...
                            idusuario, fhregistro)
                VALUES (NULL, :idventa, :idproducto, :cantidad, :unidad,
                            :pventa, :igv, :icbper, :descuento, :total, :idafectacion, 
                            :estado, :idusuario, NOW())";
        global $cnx;
        $pre = $cnx->prepare($sql);
    ...
    function reporteVentasPorComprobante($desde, $hasta){
        $sql = "SELECT YEAR(v.fecha) anio, MONTH(v.fecha) mes, t.idtipocomprobante, SUM(v.total) total
        FROM venta v 
        ...
        $sql.=" GROUP BY anio, mes, idtipocomprobante
        ORDER BY anio ASC, mes ASC, idtipocomprobante ASC";
        global $cnx;
    ...
    function TotalVentasDelMes(){
        $sql = "SELECT SUM(v.total) total FROM venta v WHERE v.estado=1";
        global $cnx;
    ...
    function CantidadVentasDelMes(){
        $sql = "SELECT COUNT(v.total) total FROM venta v WHERE v.estado=1";
        global $cnx;
        $resultado = $cnx->query($sql);
        if($resultado->rowCount()>0){
            $resultado = $resultado->fetch(PDO::FETCH_NUM);
