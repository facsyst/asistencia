...
class Personal{
    ...
    function listar($nombre, $nrodoc, $estado){
        $sql = "SELECT p.idpersonal, p.nombre, t.nombre  tipodoc, p.nrodocumento, p.tipopersonal, p.celular, p.email, p.estado
                FROM personal  p INNER JOIN tipodocumento t ON p.idtipodocumento =t.idtipodocumento 
                WHERE p.estado <2
                ";
        $parametros = array();
        if($nombre!=""){
